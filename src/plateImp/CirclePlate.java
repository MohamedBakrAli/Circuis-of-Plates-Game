...
public class CirclePlate extends ObservableClass implements PlateIF {
	...
	public CirclePlate(Color color) {
	...
	public PlateIF clone() {
		PlateIF newCircle = new CirclePlate(this.color);
		newCircle.setPosition((int) this.getCurrentPosition().getX(), (int) this.getCurrentPosition().getY());
		newCircle.setMove(this.onStack);
		return newCircle;
	}
